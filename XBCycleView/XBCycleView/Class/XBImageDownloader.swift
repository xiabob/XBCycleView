...
public class XBImageDownloader: NSObject {
    ...
    private func getLastName(rawString: String) -> String {
        let url = rawString.componentsSeparatedByString("/").last
        if url != nil {
            return url!
        } else {
            return ""
    ...
    private func getImageFromLocal(url: String) -> UIImage? {
        let path = imageCacheDir() + "/" + getLastName(url)
        ...
        dispatch_async(dispatch_get_global_queue(0, 0)) { [unowned self] in
            if let imageUrl = NSURL(string: url) {
                if let imageData = NSData(contentsOfURL: imageUrl) {
                    //save to disk
                    imageData.writeToFile(self.imageCacheDir() + "/" + self.getLastName(url), atomically: true)
